...
select v.dtVenda 'Data', p.nome 'Nome', cr.quantidade 'Quantidade'
	from vendas as v join carrinho as cr
		on v.idVenda = cr.fkVenda
			join produto as p
				on p.idProduto = cr.fkProduto
					where idVenda = 1;
...
select p.nome, p.preco, sum(quantidade) 'Quantidade'
	from produto as p join carrinho as cr
		on cr.fkProduto = idProduto
			group by p.nome;
...
-- CORREÇÃO
select sum(distinct preco) 'Preço' from produto;
